...
int main(int argc, char **argv) {
    ...
    int top = 16;	/* TOP 10 solutions. They are better if they acomplish more preferences and
...
//		printf ("Infeasibe --------------------------------------->\n");
   ...
   if (make_prune != -1) { /* If it was feasible we count it as a solution */
	...
	n_solution += 1;	
//		printf("Solution %i \n", n_solution);
printf("Hola caracola X\n\n");
	}
   }
   current_try += 1;	/* Try again */
printf("Hola caracola Y\n\n");
}
...
*/printf("Hola caracola\n\n");
	for (i = 0; i < n_top; i++) {
		...
		top_solutions[i].n_holes = aux_holes;
printf("Hola caracola\n\n");
		top_solutions[i].holes = (int *) malloc ( sizeof(int)*aux_holes);

		if (top_solutions[i].holes == NULL) {
			perror ("Error al reservar memoria");
		}
...
/* -------------------->  PRINT Solution <-----------------------------*/
	if (n_solution == 0) {
		printf("Ninguna solucion es posible para la semana %i \n",w);
		return 1;
	}
/*
	for (i = 0; i < n_top; i++){
...
/* Free memory we wont use again */
	...
	free (solutions);
	free (preferences);
/*---------------------------- PREPARE THE SOLUTIONS STRUCTURES TO BE MORE EFFICIENTLY RESOLVED--------------- */
...
for (i = 0; i < weeks; i++ ) {
  for (j = 0; j < top_week[i]; j++) {
    for (k = 0; k < total_solutions[i][j].n_holes; k++) {	
		hole = total_solutions[i][j].holes[k];
/* For every hole of every week we check if there is a possible doble turn that would work two days in a row */

	  for (h = max(hole - hole%(times- 1) - (times-1),0); h <= min(hole + 1 -hole%(times- 1) + (times-1),days*(times-1)); h++){
	    for (p = 0; p < total_solutions[i][j].posibles[k][0];p++){
			if (total_solutions[i][j].chose[h] == total_solutions[i][j].posibles[k][1+p]) {
				total_solutions[i][j].posibles[k][0] -= 1;
				if ( p != 0 ) {	
					for (a = p; a < total_solutions[i][j].posibles[k][0];a++) {
						total_solutions[i][j].posibles[k][a] = total_solutions[i][j].posibles[k][a+1];
					}
				}
			}
	     }
	  }
/* We check if a given hole has 1 or 0 possibilities coz in that case we remove them */
	if (total_solutions[i][j].posibles[k][0] == 1) {
		total_solutions[i][j].chose[hole] = total_solutions[i][j].posibles[k][1];
		
	}
	if (total_solutions[i][j].posibles[k][0] == 0) {	/* Infeasible week */
		printf ("Error=====================================================D\n");
	}
      }
   }
